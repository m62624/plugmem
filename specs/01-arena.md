...
`get_unchecked` разрешён только там, где границы доказаны предшествующей
проверкой в той же функции, с зеркальным `debug_assert!`. Каждый unsafe-блок —
комментарий `// SAFETY:`. Весь крейт проходит miri в CI. Функциональная
корректность сначала фиксируется safe-версией, unsafe-оптимизация — отдельным
коммитом с бенч-подтверждением.
