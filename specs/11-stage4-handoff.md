...
- Tombstone-факты **не удаляются из арены фактов** (48 байт — дёшево, id
  обязан оставаться занятым), но их полезная нагрузка вычищается:
  текст → общий пустой блоб, вектор → выбрасывается (`vector = NONE_U32`,
  флаг снят), теги → `ListHandle::EMPTY`, из всех индексов исключаются.
