...
1. `db_uuid` в формат снапшота (pre-1.0, пока формат свободен; specs/12 §6);
2. этап C — `plugmem-testgen` (нужен до HNSW: корпуса для recall-гейтов
   и замера порога `flat_to_hnsw`);
3. этап 6 — HNSW (specs/10);
